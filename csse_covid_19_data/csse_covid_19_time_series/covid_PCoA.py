...
df = dataframe_all_countries[
    (dataframe_all_countries['quantity'] == 'Confirmed')]
df = df.reset_index()
...
DailyDataFifty = DailyData[[i for i in DailyData.columns if int(DailyData[i].sum()) > 50]]
...
ax4 = fig4.subplots()
for region in RegionList:
    ax4.plot(dfs_cumulative[region][dfs_cumulative[region]>50].index,
             dfs_cumulative[region][dfs_cumulative[region]>50].values)
    ax4.annotate(region,
                 xy=(list(dfs_cumulative[region][dfs_cumulative[region]>50].index)[0],
